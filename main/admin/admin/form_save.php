...
if (!empty($_POST)) {
    ...
    if ($id > 0) {
        ...
        if ($userItem != null) {
            ...
            if ($password != '' || $avatar != '') {
                // đổi mật khẩu
                $sql = "UPDATE admin set avatar='$avatar',fullname='$fullname',email='$email',phone_number='$phone_number',address='$address',password='$password',role_id=$role_id,updated_at='$updated_at' WHERE id =$id";
                $db->execute($sql);
                $msgsuccess = "Sửa tài khoản thành công";
            } else {
                // truong hop mat khau la rỗng
                $sql = "UPDATE admin set fullname='$fullname',email='$email',phone_number='$phone_number',address='$address',role_id=$role_id,updated_at='$updated_at' WHERE id =$id";
        ...
        if ($userItem != null) {
            ...
            $sql = "INSERT INTO admin (fullname,email,phone_number,address,password,avatar,role_id,created_at,updated_at) VALUES ('$fullname','$email','$phone_number','$address','$password','$avatar',$role_id,'$created_at','$updated_at')";
            $db->execute($sql);
